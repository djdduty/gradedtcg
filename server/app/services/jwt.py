...
def create_jwt_token(
    *,
    ...
    to_encode.update(JWTMeta(exp=expire).dict())
    return jwt.encode(to_encode, secret_key, algorithm=ALGORITHM)


def create_access_token_for_user(user: User, secret_key: str) -> str:
    return create_jwt_token(
        jwt_content=JWTUser(username=user.username, sub=user.username).dict(),
...
def get_username_from_token(token: str, secret_key: str) -> str:
    try:
        ...
        ).sub  # We could also check for payload.username rather than payload.sub
